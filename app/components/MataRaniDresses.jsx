    ...
    <section className="mx-auto max-w-[1720px] w-full bg-[#F8F6F3] py-10 sm:py-14 md:py-16 lg:py-[54px]">
      <div className="mx-auto max-w-[1440px] px-6 sm:px-10 md:px-16 lg:px-24">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:mb-10 sm:flex-row sm:items-center sm:justify-between lg:mb-11">
          <h2 className="text-[24px] sm:text-[32px] md:text-[36px] lg:text-[40px] font-semibold font-playfair leading-tight text-[#303030]">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_300px] xl:grid-cols-[minmax(0,1fr)_404px] lg:gap-8">
              ...
              {products.map((product) => (
                <SwiperSlide key={product.id}>
                  <div className="overflow-hidden bg-white ring-1 ring-[#EFEAE4] h-full lg:h-[540px] flex flex-col cursor-pointer group rounded-sm shadow-sm">
                    ...
                    <div className="px-4 pb-4 pt-4 sm:px-5 sm:pb-5 flex flex-col flex-1">
                      <h3 className="text-[17px] sm:text-[19px] md:text-[21px] font-medium leading-[1.25] font-gt-walsheim text-[#303030] group-hover:text-[#7A1F3D] transition-colors line-clamp-1">
                        {product.name}
                      </h3>

                      <p className="mt-2 text-[13px] sm:text-[14px] leading-[1.45] font-gt-walsheim text-gray-600 line-clamp-2">
                        {product.short_description}
                      </p>

                      <p className="mt-3 text-[20px] sm:text-[22px] md:text-[24px] font-bold font-gt-walsheim text-[#7A1F3D]">
                        {formatPrice(product.price, product.usd_price)}
                      </p>

                      <div className="mt-auto pt-4 flex flex-col gap-2.5 min-[480px]:flex-row">
                        <Link href={`/product-details/${product.slug}`} className="flex-1">
                          <button className="w-full border border-[#7A1F3D] bg-[#7A1F3D] px-4 py-2 md:py-3 text-[13px] md:text-[14px] font-gt-walsheim font-medium text-white transition-all duration-300 hover:bg-white hover:text-[#7A1F3D] cursor-pointer">
                            Shop Now
                          </button>
                        </Link>
                        <button 
          ...
          {/* Promo card */}
          <div className="relative min-h-[300px] w-full lg:w-[300px] xl:w-[404px] h-[300px] sm:h-[400px] lg:h-[540px] overflow-hidden bg-white ring-1 ring-[#EFEAE4] rounded-sm shadow-sm">
            <img
