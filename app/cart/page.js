...
function CartContent() {
    ...
    const priceINR = searchParams.get("priceINR");
                ...
                try {
                    ...
                    if (data && data.product) {
                        ...
                        if (data.product.variations && data.product.variations.length > 0) {
                            const initialVariant = data.product.variations.find(v =>
                                (currentColor ? v.color === currentColor : true) &&
                                (currentSize ? v.size === currentSize : true)
                            ) || data.product.variations[0];
    ...
    const handleConfirmAdd = async () => {
        ...
        if (!token) {
            setStatus({ type: "error", message: "Bearer token required. Redirecting to login..." });
            setTimeout(() => {
                router.push("/login");
            }, 2000);
            return;
        ...
        // Extract user_id from stored user data with high robustness
        let userId = "";
        try {
            const userStr = localStorage.getItem("user");
            if (userStr) {
                const userData = JSON.parse(userStr);
                // Check all common field names for ID
                userId = userData?.id || userData?.user_id || userData?.userid || userData?.ID || "";
                
                // If it's still empty and the data itself is a number (it shouldn't be, but just in case)
                if (!userId && typeof userData === 'number') userId = userData;
            }
        } catch(e) { console.error("Error parsing user data", e); }

        if (!userId) {
            setStatus({ type: "error", message: "User session incomplete (Missing ID). Please Logout and Login again to refresh your account." });
            setIsAdding(false);
            return;
        }
        ...
        try {
            const payload = {
                ...
                variation_id: String(finalVariantId),
                ...
                try {
                    ...
                    if (stored) {
                        ...
                        const filtered = removed.filter(r => {
                            // Remove entries matching this product_id (use pId for consistency)
    ...
    return (
        <div className="max-w-[1440px] mx-auto px-6 sm:px-10 md:px-16 lg:px-24 py-2 md:py-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 lg:items-start">
                ...
                <div className="bg-white border border-[#E8DDD4] rounded-sm p-5 sm:p-7 shadow-sm flex flex-col h-auto">
                    <h2 className="text-xl sm:text-2xl font-playfair font-bold text-[#303030] mb-4 relative">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <div className="flex items-center gap-3 mb-2">
                                        <h3 className="text-2xl sm:text-3xl font-playfair font-semibold text-[#303030] leading-tight">{productName}</h3>
                                        {productData && (
                                ...
                                {formattedPrice && (
                                    <p className="text-3xl font-bold text-[#7A1F3D] tracking-tight">{formattedPrice}</p>
                                )}
                            </div>
                ...
                <div className="bg-[#FDF8F3] border border-[#E8DDD4] rounded-sm p-5 sm:p-7 shadow-lg flex flex-col justify-between h-auto lg:sticky lg:top-24">
                    ...
                    <div>
                        <div className="pt-4 border-t-[2px] border-[#E8DDD4] flex justify-between items-end mb-6">
                            ...
                            <span className="font-bold text-[#7A1F3D] text-3xl tracking-tight">{formattedTotalPrice || "TBD"}</span>
                        </div>
